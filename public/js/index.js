...
devices.on("click", (e) => {
  const message =
    "This field has to do with Tickets that are RMA. It will activate the radio button below and the input field for number of devices. \n\nDo you wish to continue?";
  if (confirm(message)) {
...
logTicket.on("submit", (e) => {
  // submiting form.
  if (
    $('input[name="request"').is(":disabled") &&
    $('input[name="numberDevices"').is(":disabled")
  ) {
    alert(
      "Please define if its a request or an RMA using the Request or Number of device button."
    );
    return false;
    e.preventDefault();
  } else if ($("#summary").val() == "") {
